...
export default function SalesPage() {
    ...
    const handleCreateOrder = async (e) => {
        ...
        try {
            const payload = {
                tableNumber: String(orderForm.tableNumber).trim(),
                paymentMethodType: String(orderForm.paymentMethodType).trim(),
                items: orderForm.items
                    .map((i) => ({
                        productName: i.productName.trim(),
                        quantity: Number(i.quantity),
                        notes: i.notes?.trim() || null,  // null thay vì "" để tránh validation fail
                    }))
                    .filter((i) => i.productName && i.quantity > 0),
            };

            console.log("[createOrder] payload gửi lên:", JSON.stringify(payload, null, 2));
    ...
    const handleAddProductDirectly = async (product) => {
        if (!selectedOrderId) {
            setActionError("Vui lòng chọn một hóa đơn đang mở trước!");
        ...
        if (!currentOrder || currentOrder.status !== "OPEN") {
            setActionError("Hóa đơn này đã đóng, không thể thêm món!");
            setTimeout(() => setActionError(""), 3000);
            return;
        }
        ...
        try {
            ...
            setActionMessage(`Đã thêm ${product.productName} vào đơn hàng`);
                                ...
                                {loadingProducts ? (
                                    ...
                                    products.map((p) => (
                                        <div className="col-6" key={p.productID}>
                                            <div
                                                className="border rounded p-2 d-flex gap-2 align-items-center bg-white h-100 product-card"
